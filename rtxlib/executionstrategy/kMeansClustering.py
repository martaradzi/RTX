...
def start_k_menas_clustering_strategy(wf):
    ...
    # wandb.init(project='sequential_k-means_clustering_overhead_test', name="test_run_1")
    kmeansModel = kMeans()

    number_of_submodels_trained = 1

    data = []   # stored data for visualization purposes
    data_for_partial_clustering = []    # temp array for storing data on the go
    data_for_clutering = []             # temp array for visualizing partial clustering 
    sample_number = 0                   # number of samples porcessed. Sometimes samples are empty hence there is a need for tracking
        ...
        if new_sample is not None:
            ...
            if len(data_for_partial_clustering) == partial_clustering_size:
                ...
                if number_of_submodels_trained == 1:
                    data_for_clutering = data_for_partial_clustering
                    kmeansModel.fit(data_for_partial_clustering)
                else:
                    data_for_clutering = np.concatenate([data_for_clutering, data_for_partial_clustering], axis=0)
                ...
                plt.savefig(folder + str(number_of_submodels_trained) +'_med_90thpercentile.png')
                plt.close()
        sample_number += 1
